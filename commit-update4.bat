...
set "LOG=%~dp0commit-update4-log.txt"

echo. > "%LOG%"
echo === commit-update4.bat === >> "%LOG%"

"%GIT_EXE%" add -A >> "%LOG%" 2>&1
echo Add exit: %ERRORLEVEL% >> "%LOG%"

"%GIT_EXE%" commit -m "Update 4: fix poll-average inflation; deploy NC county baseline + calibration" >> "%LOG%" 2>&1
