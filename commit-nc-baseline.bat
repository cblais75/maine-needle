...
set "LOG=%~dp0commit-nc-log.txt"

echo. > "%LOG%"
echo === commit-nc-baseline.bat === >> "%LOG%"

"%GIT_EXE%" add data/nc-baseline.json >> "%LOG%" 2>&1
echo Add exit: %ERRORLEVEL% >> "%LOG%"

"%GIT_EXE%" commit -m "Add NC county baseline data" >> "%LOG%" 2>&1
