// build-baseline-nc.mjs — generate North Carolina's county baseline from real past results.
//
...
(async () => {
  ...
  const dest = path.join(ROOT, "data", "nc-baseline.json");
  fs.writeFileSync(dest, JSON.stringify(baseline, null, 2));
  console.log(`Wrote ${dest}`);
  console.log(`Counties: ${counties.length} (expect 100) · statewide D two-party: ${(out.stateShare * 100).toFixed(1)}%`);
  if (counties.length && counties.length !== 100) console.log("NOTE: not 100 counties — check the source file / column matching before using.");
})();
