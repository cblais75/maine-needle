...
set "LOG=%~dp0commit-polls-log.txt"

echo. > "%LOG%"
echo === commit-polls.bat === >> "%LOG%"

"%GIT_EXE%" add data/polls.json >> "%LOG%" 2>&1
echo Add exit: %ERRORLEVEL% >> "%LOG%"

"%GIT_EXE%" commit -m "Add new June 2026 polls (Maine Senate, NC Senate)" >> "%LOG%" 2>&1
