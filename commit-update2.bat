...
set "LOG=%~dp0commit-update2-log.txt"

echo. > "%LOG%"
echo === commit-update2.bat === >> "%LOG%"

"%GIT_EXE%" add -A >> "%LOG%" 2>&1
echo Add exit: %ERRORLEVEL% >> "%LOG%"

"%GIT_EXE%" commit -m "Update 2: North Carolina county-level baseline (100 counties)" >> "%LOG%" 2>&1
echo Commit exit: %ERRORLEVEL% >> "%LOG%"

"%GIT_EXE%" log --oneline -4 >> "%LOG%" 2>&1

:: Token push (same method as git-push-final.bat)
